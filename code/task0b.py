...
OUTPUT_FOLDER = "./outputs/vectors/"

# This is the total number of unique features in our dataset, so our vector size will be this for every database
# object or query object
feature_dict = set()
...
def get_number_of_gesture_files_in_dir(directory_path):
    return len([x for x in os.listdir(directory_path) if x.endswith(".csv")])
...
def process_word_file(directory_path, file_name):
    ...
    for row in words:
        word = [row[0], row[1]]  # component id, sensor id
        word.extend(row[5:])  # quantized values
        word = tuple(word)
...
def create_tf_vectors(feature_list):
    # Creating TF vectors
    tf_vector_output_file = open(OUTPUT_FOLDER + "tf_vectors.csv", "w")
...
def create_tf_idf_vectors(gestures_dir, feature_list):
    ...
    tf_vectors = np.array(pd.read_csv(OUTPUT_FOLDER + "tf_vectors.csv", header=None))
    ...
    # print(np.any((idf_vector == 0))) # VERIFY IF ANY VALUE EQUALS ZERO
    tf_idf_vectors = tf_vectors[0:, 1:] * idf_vector
    tf_idf_vectors = np.hstack((tf_vectors[0:, :1], tf_idf_vectors))
    pd.DataFrame(tf_idf_vectors).to_csv(OUTPUT_FOLDER + "tf_idf_vectors.csv", header=None, index=None)
...
if __name__ == '__main__':
    ...
    create_tf_vectors(feature_list)
