...
if __name__ == '__main__':
    ...
    if args.type == 1:
        ...
        joblib.dump(pca, args.output_dir+"pca.sav")
        ...
        pd.DataFrame(latent_vectors).to_csv(args.output_dir + "pca_vectors.csv", header = None, index = None)
    elif args.type == 2:
        svd = TruncatedSVD(n_components=args.k)
        svd.fit(vectors[0:, 1:])
        # saving the model, so that it can be used in future tasks to transform the query gesture
        joblib.dump(svd, args.output_dir+"svd.sav")
        ...
        pd.DataFrame(latent_vectors).to_csv(args.output_dir + "svd_vectors.csv", header = None, index = None)
    elif args.type == 3:
        nmf = NMF(n_components=args.k)
        nmf.fit(vectors[0:, 1:])
        # saving the model, so that it can be used in future tasks to transform the query gesture
        joblib.dump(nmf, args.output_dir+"nmf.sav")
        ...
        pd.DataFrame(latent_vectors).to_csv(args.output_dir + "nmf_vectors.csv", header = None, index = None)
    elif args.type == 4:
        lda = LatentDirichletAllocation(n_components=args.k)
        lda.fit(vectors[0:, 1:])
        # saving the model, so that it can be used in future tasks to transform the query gesture
        joblib.dump(lda, args.output_dir+"lda.sav")
        ...
        pd.DataFrame(latent_vectors).to_csv(args.output_dir + "lda_vectors.csv", header = None, index = None)
    else:
